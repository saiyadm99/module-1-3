...
// create async thunk
const fetchRelatedPosts = createAsyncThunk("post/fetchRelatedPosts", async(value) => {
	const titleString = value;

	var titles = titleString.split(" ");

  let queryString = titles.map((title) => `title_like=${title}`).join("&");						
